...
# A single URL
uv run scripts/08_archive_cite.py --url "https://lda.senate.gov/filings/public/filing/..."

# A file of URLs (one per line)
uv run scripts/08_archive_cite.py --urls notes/extra_urls.txt

# Service-specific runs
uv run scripts/08_archive_cite.py --service wayback
uv run scripts/08_archive_cite.py --service archive_today

# Skip URLs successfully archived in the last N days (default 30)
uv run scripts/08_archive_cite.py --skip-recent 7
...
uv run scripts/08_archive_cite.py --skip-recent 30
```
