...
uv run scripts/05_pressrel_search.py --mention "Cargill" \
...
uv run scripts/05_pressrel_search.py \
    --case skill/press-release-cross-ref/cases/limbaugh_clients.json \
...
- `skill/press-release-cross-ref/cases/steinberg_clients.json` — 9 clients,
  ~59 verified matches across ~37 members (numbers depend on which version of
  the corpus is loaded; the structure is stable).
- `skill/press-release-cross-ref/cases/limbaugh_clients.json` — 8 clients,
