...
description: >
  ...
  attention — the highest-signal cases), and **bridges** (clients shared by two
  or more independent revolving-door cases — possible focal points for a wider
  story). Emits the graph as JSON adjacency for the Reporter UI's /graph route,
  a static SVG for the findings PDF, a Graphviz DOT for users who want to
  compile their own layouts, and a markdown summary listing the most
  newsworthy triangles/hubs/bridges. Use whenever you want to *see* the
  cross-source connections rather than read three separate reports, or any
  time someone asks "what's the actual chain here?"
version: 1.0.0
...
# Narrower scope
uv run scripts/06_coi_graph.py --top 5

# A single finding's subgraph (rank N from findings.json)
uv run scripts/06_coi_graph.py --finding-rank 1

# Persist outputs
uv run scripts/06_coi_graph.py --out notes/coi_summary.md

# CI / offline mode
uv run scripts/06_coi_graph.py --no-web --no-svg
```
