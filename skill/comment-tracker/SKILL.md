...
```bash
# Read
uv run scripts/07_comment_tracker.py list                  # all firms with status + days
uv run scripts/07_comment_tracker.py status venn_strategies # one firm, with timeline
...
uv run scripts/07_comment_tracker.py log venn_strategies acknowledged \
    --by "Press Office" --summary "Acknowledged receipt, working on response"

uv run scripts/07_comment_tracker.py log venn_strategies substantive_reply \
...
uv run scripts/07_comment_tracker.py log venn_strategies followup_sent \
    --addresses press@vennstrategies.com

uv run scripts/07_comment_tracker.py log venn_strategies closed \
    --kind response --summary "Used the brief statement in print"

uv run scripts/07_comment_tracker.py log acme_corp legal_threat \
...
uv run scripts/07_comment_tracker.py export                # writes web/public/comment_log.json
```
