...
license: MIT
compatibility: Requires Python 3.11+ and uv. Run all commands with `uv run`.
metadata:
...
uv run scripts/01c_rebuild_house_all.py
uv run scripts/01_build_index.py --duckdb-only

# Remove stale Parquet then rebuild everything
uv run scripts/01_build_index.py --clean
```
...
See [`references/senate_schema.md`](references/senate_schema.md) and
[`references/house_schema.md`](references/house_schema.md) for full column-level documentation.
...
1. **"Miss" on senate JSON files, Total rows: 0** — Expected if Parquet files
   already exist. The script loads from Parquet and skips re-parsing. DuckDB
   row counts will still be correct.
...
2. **Schema inference errors on House Parquet** — House XML has all-null columns
   ...
   See [`references/known_quirks.md`](references/known_quirks.md) §4.
...
4. **DuckDB type mismatch on union_by_name** — Means the first Parquet file in a
   glob has a different inferred type than later ones. Fix: explicit
...
**Read [`references/known_quirks.md`](references/known_quirks.md) before modifying any parsing code.**

Seven parser bugs that produce silent data loss are documented there. Each one
...
See [`references/joins.md`](references/joins.md) for how to link Senate ↔ House ↔ press releases.
