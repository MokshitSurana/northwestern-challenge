...
clients. Run `uv run skills/federal-award-tracer/scripts/04_award_tracer.py --print-template` to see the schema.
...
uv run skills/federal-award-tracer/scripts/04_award_tracer.py --print-template
...
uv run skills/federal-award-tracer/scripts/04_award_tracer.py \
  --case skills/federal-award-tracer/cases/limbaugh_interior.json \
...
| `cases/limbaugh_interior.json` | Limbaugh's water-district clients → Interior grants | $133,119,412 (grants only, 18 clients; Glenn-Colusa Reclamation award reported separately — see notes/08) |
